...
namespace UtmostInput
{
    public class InputX
    {
        List<GeneralMOInput> generalMInputs;
        ...
        public InputX()
        {
            generalMInputs = new List<GeneralMOInput>();
        ...
        public bool SetInputs()
        {
            ...
            if (MOExtendedInputs.isInputEntered() && generalMInputs.Count == 0) // for now works with only one input
            {
                ...
                generalMInputs.Add(tmpGi);

                return true;
            }
            else if (MOExtendedInputs.isInput())
            {
                GeneralMOInput tmpGi = generalMInputs[0];
                ...
                generalMInputs[0] = tmpGi;

                return true;
            }
            else if (generalMInputs.Count > 0)// && isinput değilse 
            {
                GeneralMOInput tmpGi = generalMInputs[0];
        ...
        {
            if (generalMInputs.Count > 0 && generalMInputs[0].phase == IPhase.Ended)
            {
                generalMInputs.RemoveAt(0);
            }
        }

        public GeneralMOInput GetInput(int index)
        {
            return generalMInputs[index];
        }
        ...
        /// </summary>
        /// <returns></returns>
        public Vector2 GetMousePositionInCoordinates()
        {
            Vector2 mousePos = GetMouseCurserPosition();
